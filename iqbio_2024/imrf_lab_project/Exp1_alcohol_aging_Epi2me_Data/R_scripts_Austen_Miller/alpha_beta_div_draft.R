...
# What to do with unclassified species?
# What kind of distance matrix to use?
# What kind of ordination?
# Uhh help me understand the ordination plot?
# Ask about accuracy

# rarefaction curve
# do weighted and unweighted unifrac
# Get rid of singltons (and more?) and run abundance without them
# Joinup barcode samples at beginning?
# Fastmode was used for basecalling instead of superaccuracy (ugh)
  # Should we cut out some of these with low accuracy?
# Pore activity graph showed many unavailable for sequencing (not 100% on how that is supposed to work though)
...
# metadata with:
  # sample id
  # expositions to alcohol
    # yes/no, another column 1,2,0
  # taxa
  # relative abundance


# Which taxonomic ranks should be merged/ignored?
  # combine subspecies and strain? Kingdom and Superkingdom?
  # should we even be using subspecies and strains? A few are being counted seperate
  # from the same species without the strain
  # which way of assigning genus to use?

# Removing strain and subspecies for now. They're causing a bunch of problems

# What to do with Eukaryotic species?
  # Shouldn't 16s not pick up any of those?
  # It's almost certainly some kind of problem with the taxize functions or NCBI
  # It's because of the strain names on the species

# Should we get rid of more than singletons?

# What database to use for taxonomy?

# What to do with species that didn't get assigned?
  # Seem to be more because of the strain numbers in the species names

# What to do exactly with relative abundance?

# Should the rarefaction curve be done by sample group or by barcode?
...
# To cite the packages just use the citation function and it'll output in the console
citation("tidyverse")
citation("vegan")
citation("taxize")
...
# Filter the unclassifieds?
# class_data_filtered <- class_data %>%
#   filter(exit_status != "Unclassified")
# 
# # Wait, just how many of the unclassified species did the filter get rid of?
# count(class_data)- count(class_data_filtered) # 615 unclassified removed
# 615/74803 # 0.82% isn't a huge amount
# Juanse suggested not getting rid of them because they do represent data

# Edit: Filter out barcode 23 (some kind of control) and unclassified barcode
  # Imilce said that the unclassified was some kind of extra sample, not relevant for us.
class_data_filtered <- class_data %>% 
  filter(!barcode %in% c("unclassified","barcode23"))


# We need to filter out the singletons
...
# The empty strings on the unclassified stuff are creating problems in the diversity calculation.
...
# Edit: Add in the Sample IDs column based on barcodes
...
#stringr::str_replace and str_detect replace and detect parts of strings. Very useful.
class_data_format <- class_data_format %>%
  mutate(etoh_exp = as.numeric(case_when( # also needs to be numeric
...
# This is a very annoying problem during taxonomic assignment.
...
# Will have to add their column back to all the select functions throughout the
# script if we change this...
...
#### Assigning Genus with species name ####

# Hmm it could be cool to fill the genus column using the species names
# Just take the first of the Linnaean name and assume it's the genus?
# could also get an alpha diversity of genera. That's a thing, right?

# Okay so Carlos already made genus plots, however...
# I think he may have thrown out a lot of data without realizing it
# when he made his plots, he used this:
...
# # Are we throwing out all the unassigned genera?

# I think we could assign most, if not all of these genera

# Just how many NAs are there?
sum(is.na(class_data_format$genus)) # 46224 NAs
sum(!is.na(class_data_format$genus)) # 23604 non NAs
46224/69828 # that's 66% of the data that isn't being used in the compositions

# Okay we want to specifically change the NA genus values, not the others
# Let's find if any of those have non Linnean species names
na_species <- class_data_format %>% 
  filter(is.na(genus)) %>% 
  select(species) %>% 
  unique() #There are 263 species without assigned genus

species_unique <- class_data_format %>% 
  select(species) %>% 
  unique() # ...out of a total of 354 (was just curious)
...
# Okay let's assign genus
class_data_format <- class_data_format %>%
  mutate(genus = ifelse(is.na(genus), word(species, 1), genus))
# View(class_data_format) # That appears to have worked... and was easier than I thought

# Replace empty strings in the species column with "unclassified" (problems later with empty strings)
class_data_format$genus <- ifelse(class_data_format$genus == "", "Unclassified", class_data_format$genus)

## Relative Abundance Table ####

# Get rid of these extra columns
class_data_relative_abundance <- class_data_format %>% 
  select(barcode, sample_id, etoh_exp, age, strain_subspecies, species, genus)


#### Taxonomic Assignment ####

# Let's see if we can get the full taxonomy of these species
# Taxize can do it. Time to get Hal 9000 to teach me how to use taxize.
...
# Do I totally understand this function? No. But doing this with just classification() sucks.
...
# I'm not gonna pretend I fully understand this function and loop, but it was super annoying
# trying to this with just classification(). This way it cycles through all of the species,
# has robust error handling, and pivots the dataframes with their tax info.
...
# Oh right, two different genera column
# Idk which to use... so let's compare them
class_data_relative_abundance$genus_comparison_result <- class_data_relative_abundance$genus.y == class_data_relative_abundance$genus.x
class_data_relative_abundance %>% 
  filter(genus_comparison_result == FALSE) %>% 
  count() # the assigned genera look more accurate

# Reorder the columns for readability and differentiate the two versions of genus
class_data_relative_abundance$genus_by_species_name <- class_data_relative_abundance$genus.x
class_data_relative_abundance$genus_assigned <- class_data_relative_abundance$genus.y

class_data_relative_abundance_BothGenera <- class_data_relative_abundance %>% 
  select(barcode, age, etoh_exp, species, genus_by_species_name, genus_assigned,
         genus_comparison_result, family, order, class, phylum, superkingdom) # this is a copy with both versions of genus just in case

# If you're using a dataset that doesn't include some of these ranks here, you'll
# get an error like "column (whatever rank) doesn't exist". Just delete that rank
# from the rank list above the loop, and this select function. That should resolve the error.

# This'll be the main copy for now
class_data_relative_abundance <- class_data_relative_abundance %>% 
  mutate(genus = genus_assigned)
...
# Without knowing what to do with the other ranks atm, we'll just do species to phylum for now
...
# Genus
# We're using the assigned one because the names one keeps making mistakes on
# a specific species. Rewrite if change back.

# Find total for genus per barcode (species name)
class_data_relative_abundance <- class_data_relative_abundance %>% 
  group_by(barcode, genus) %>% 
  mutate(genus_per_barcode = n())

# Calculate relative abundance for genus (species name)
class_data_relative_abundance <- class_data_relative_abundance %>% 
...
# Ugh they're out of order again
colnames(class_data_relative_abundance)
class_data_relative_abundance <- class_data_relative_abundance %>% 
...
species_matrix <- class_data_relative_abundance %>%
  ...
  summarise(count = n(), .groups = 'drop')# Create column with how many occurrences of each species in each barcode
...
species_matrix <- species_matrix %>% 
  ...
  # values_fill = list(count = 0)) to make it use 0 instead of NA. Annoying otherwise
...
# write.csv(class_data_relative_abundance, "~/Desktop/IMRF_alcohol_relative_abundance_table.csv")
# write.csv(species_matrix, "~/Desktop/IMRF_alcohol_species_matrix.csv")
...
# Do I know how anonymous functions work? Uh... sort of.
# But my good friend skynet can teach me!
...
# Make it easier to differentiate by age by adding age column
alpha_diversity <- alpha_diversity %>% 
  mutate(age = case_when(
    sample_id %in% c("young_5d_0_EtOH_exposition", "young_5d_1_EtOH_exposition", 
                     "young_5d_2_EtOH_exposition") ~ "young (5 days)",
...
# Don't really need the other indexes (for now)
alpha_diversity <- alpha_diversity %>% 
  select(c(barcode, age, etoh_exp, shannon))

# These ugly names are getting really annoying
#stringr::str_replace and str_detect replace and detect parts of strings. Very useful.
alpha_diversity <- alpha_diversity %>%
...
alpha_diversity %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
        ...
        panel.border = element_rect(colour = "black", fill = NA, linewidth = 1),
        plot.title = element_text(size = 16, face = "bold")) +
  labs(title = "Alpha Diversity By Age (Species)", y = "Shannon Diversity Index", x = "Age")

# Remember ggsave() saves the last generated plot by default
# Change the file name so it doesn't overwrite the last one
ggsave("./plots/species_boxplot_age_diversity.png")

# Microbial Composition Plot (Species)
...
# Hmm it wants a color for every single of the bajilion species
...
species_counts <- class_data_relative_abundance_Sgrouped %>% 
...
custom_labels <- c("0x (Young)", "1x (Young)", "2x (Young)", "0x (Old)", "1x (Old)", "2x (Old)")
...
alpha_diversity_genus %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
        ...
        panel.border = element_rect(colour = "black", fill = NA, linewidth = 1),
        plot.title = element_text(size = 16, face = "bold")) +
  labs(title = "Alpha Diversity By Age (Genus)", y = "Shannon Diversity Index", x = "Age")

# Remember ggsave() saves the last generated plot by default
# Change the file name so it doesn't overwrite the last one
ggsave("./plots/genus_barcode_age_diversity.png")



# Microbial Composition Plot (Genus)
...
# Custom labels (interaction() makes them ugly otherwise)
custom_labels <- c("0x (5 Days)", "1x (5 Days)", "2x (5 Days)", "0x (50 Days)", "1x (50 Days)", "2x (50 Days)")
...
# # At this point they appear to have matching rows. Awesome
# rownames(tax_table) <- tax_table$species # species as rownames
# 
# 
...
# 
# taxa_names(otu_table) == taxa_names(tax_table) # Finally it works
# # It appears that the data frames have to both have the species as rownames
